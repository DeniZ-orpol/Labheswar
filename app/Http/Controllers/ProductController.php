                ...
                foreach ($rows as $row) {
                    if (!empty($row[0]) || !empty($row[1])) { // Check if name exists
                        ...
                        $data = [
                            ...
                            'sale_rate_a' => $row[12] ?? 0,
                            'sale_rate_b' => $row[13] != '' ? $row[14] : 0,
                            // 'sale_rate_c' => $row[15] != '' ? $row[15] : 0,
                            'sale_online' => $row[14] != '' ? $row[16] : 0,
                            'converse_carton' => $row[15] ?? 0,
                            'carton_barcode' => $row[16] ?? null,
                            'converse_box' => $row[17] ?? 1,
                            'box_barcode' => $row[18] ?? null,
                            'negative_billing' => $row[19] ?? 'NO',
                            'min_qty' => $row[20] ?? 0,
                            'reorder_qty' => $row[21] ?? 0,
                            'discount' => $row[22] ?? null,
                            'max_discount' => $row[23] ?? 0,
                            'discount_scheme' => $row[24] ?? null,
                            'bonus_use' => $row[25] == 'yes' ? 1 : 0,
                            'price_1' => $row[26] ?? null,
                            'price_2' => $row[27] ?? null,
                            'price_3' => $row[28] ?? null,
                            'price_4' => $row[29] ?? null,
