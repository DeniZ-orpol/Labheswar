...
<div class="content">
    ...
    <form action="{{ route('stockissue.update', $stockIssue->id) }}" method="POST">
        @csrf
        @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <div class="search-dropdown">
                        <input id="party_name" type="text" name="party_name_display" class="form-control search-input"
                            ...
                            value="{{ old('party_name_display', $stockIssue->getRelation('ledger')->party_name) }}">
                        <div class="dropdown-list" id="partyDropdown"></div>
                        <input type="hidden" name="ledger" id="hidden_party_id" value="{{ old('ledger', $stockIssue->ledger) }}">
                    </div>
                </div>
            </div>
            <div class="column">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="date" id="date" class="form-control date" required
                        value="{{ old('date', $stockIssue->date->format('Y-m-d')) }}">
                </div>
            </div>
            <div class="column">
                <!-- direct receipt no -->
                <div class="mb-3">
                    ...
                    <input type="text" name="issue_no" id="issue_no" class="form-control issue_no" required
                        value="{{ old('issue_no', $stockIssue->issue_no) }}">
                </div>
            </div>
        </div>
        <div class="block w-full">
            <table class="w-full mb-4 bg-transparent table-striped mt-4" id="billTbl">
                ...
                <tbody class="quote-item-container table-body" id="product-table-body">
                    @foreach(old('product_id', $stockIssue->products ? array_column($stockIssue->products, 'product_id') : []) as $index => $productId)
                    <tr class="border-b text-gray-700 uppercase text-sm quote-item-row" data-index="{{ $index }}" data-final-amount="{{ old('amount.' . $index, $stockIssue->products[$index]['amount'] ?? 0) }}">
                        <th scope="col" class="required w-1/5">
                            <div class="product-search-container" style="position: relative;">
                                <input type="text" name="product_search[]" class="form-control product-search-input field-new product_search"
                                    ...
                                    value="{{ old('product_search.' . $index, $stockIssue->products[$index]['product_search'] ?? '') }}">
                                <input type="hidden" name="product_id[]" class="product_id" required value="{{ $productId }}">
                        ...
                        <th scope="col" class="required">
                            <input type="number" name="qty[]" class="form-control qty" onchange="calculateRowAmount(this)" required min="0"
                                value="{{ old('qty.' . $index, $stockIssue->products[$index]['qty'] ?? '') }}">
                        </th>
                        <th>
                            <input type="number" name="amount[]" class="form-control field-new text-end" maxlength="255" readonly step="0.01"
                                value="{{ old('amount.' . $index, $stockIssue->products[$index]['amount'] ?? '') }}">
                        </th>
            ...
            <div class="p-5 flex justify-end">
                <div class="text-left">
                    ...
                    <input id="total-amount" type="number" step="0.0001" name="total_amount"
                        ...
                        value="{{ old('total_amount', $stockIssue->total_amount) }}">
                </div>
        ...
        <div class="row">
            <div class="column">
                <!-- Submit Button -->
                <div class="">
                    <a href="{{ route('directreceipt.index') }}" class="btn btn-outline-primary shadow-md mr-2">Cancel</a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function initProductSearch(productSearchInput) {
            ...
            function searchProducts(searchTerm) {
                fetch(`{{ route('formula.search') }}?search=${encodeURIComponent(searchTerm)}`, {
                    headers: {
            ...
            function displayProducts(products) {
                ...
                products.forEach((product, index) => {
                    ...
                    item.innerHTML = `<div class="product-name">${product.product.product_name}</div>`;
            ...
            function selectProduct(formula,product) {
                isSelecting = true;
