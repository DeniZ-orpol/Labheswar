...
<div class="content">
    ...
    <form action="{{ route('directreceipt.update', $directReceipt->id) }}" method="POST">
        @csrf
        @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <div class="search-dropdown">
                        <input id="party_name" type="text" name="party_name_display" class="form-control search-input"
                            ...
                            value="{{ old('party_name_display', $directReceipt->getRelation('ledger')->party_name) }}">
                        <div class="dropdown-list" id="partyDropdown"></div>
                        <input type="hidden" name="ledger" id="hidden_party_id" value="{{ old('ledger', $directReceipt->ledger) }}">
                    </div>
                </div>
            </div>
            <div class="column">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="date" id="date" class="form-control date" required
                        value="{{ old('date', $directReceipt->date->format('Y-m-d')) }}">
                </div>
            </div>
            <div class="column">
                <!-- direct receipt no -->
                <div class="mb-3">
                    <label for="dr_no" class="form-label">Direct Receipt No<span style="color: red;margin-left: 3px;"> *</span></label>
                    <input type="text" name="dr_no" id="dr_no" class="form-control dr_no" required
                        value="{{ old('dr_no', $directReceipt->dr_no) }}">
                </div>
            </div>
        </div>
        <div class="block w-full">
            <table class="w-full mb-4 bg-transparent table-striped mt-4" id="billTbl">
                ...
                <tbody class="quote-item-container table-body" id="product-table-body">
                    @foreach(old('product_id', $directReceipt->products ? array_column($directReceipt->products, 'product_id') : []) as $index => $productId)
                    <tr class="border-b text-gray-700 uppercase text-sm quote-item-row" data-index="{{ $index }}" data-final-amount="{{ old('amount.' . $index, $directReceipt->products[$index]['amount'] ?? 0) }}">
                        <th scope="col" class="required w-1/5">
                            <div class="product-search-container" style="position: relative;">
                                <input type="text" name="product_search[]" class="form-control product-search-input field-new product_search"
                                    ...
                                    value="{{ old('product_search.' . $index, $directReceipt->products[$index]['product_search'] ?? '') }}">
            ...
            <div class="p-5 flex justify-end">
                <div class="text-left">
                    ...
                    <input id="total-amount" type="number" step="0.0001" name="total_amount"
                        ...
                        value="{{ old('total_amount', $directReceipt->total_amount) }}">
                </div>
