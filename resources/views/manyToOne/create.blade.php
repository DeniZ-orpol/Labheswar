...
@section('content')
    <div class="content">
        ...
        <form action="{{ route('many-to-one.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-12 gap-2 grid-updated">
                ...
                <div class="input-form col-span-6 mt-3">
                    ...
                    <div class="search-dropdown">
                        <input id="ledger_name" type="text" name="ledger_name_display" class="form-control search-input"
                            placeholder="Search or type ledger name" autocomplete="off" required>
                        <input type="hidden" id="hidden_ledger_id" name="hidden_ledger_id" class="ledger_id" required>
                ...
                {{-- <div class="input-form col-span-4 mt-3">
                    ...
                    <input id="entry_no" type="text" name="entry_no" class="form-control field-new"
                        placeholder="Entry NO" maxlength="255">
                </div> --}}

                <!-- Select Product to produce or formula -->
                <div class="input-form col-span-8">
                    <label for="product_search" class="form-label w-full flex flex-col sm:flex-row">
                        Produce Item<span style="color: red;margin-left: 3px;"> *</span>
                    </label>
                    <div class="product-search-container" style="position: relative;">
                        <input id="product_search" type="text" name="product_search"
                        ...
                        <input type="hidden" id="product_quantity" value="0">
                        ...
                        <tr class="border-b text-gray-700 uppercase text-sm quote-item-row" data-index="0">
                            <th scope="col" class="required">
                                <div class="product-search-container" style="position: relative;">
                                    ...
                                    <input type="hidden" id="raw_product_id" name="raw_product_id[]"
                                        class="product_id add-product-search-input" required>
                                    <div id="raw_product_dropdown" class="product-dropdown" style="display: none;">
                                        <div class="dropdown-content"></div>
                                    </div>
                                </div>
                            </th>
                            <th scope="col" class="required">
                                <input type="number" name="raw_quantity[]" id="raw_quantity" class="form-control qty"
                                    required min="0">
                            </th>
    ...
    <!-- Produce product Quantity Modal -->
    <div id="produce-qty-modal" class="modal" aria-hidden="true" style="z-index: 50">
        <div class="modal-dialog">
            <div class="modal-content">
                <!-- BEGIN: Modal Header -->
                <div class="modal-header">
                    <h2 class="font-medium text-base mr-auto">Produce Item Quantity</h2>
                </div>
                <!-- END: Modal Header -->
                ...
                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                    <div class="col-span-12">
                        <label for="modal-produce-product-name" class="form-label">Product Name</label>
                        <input id="modal-produce-product-name" name="modal_produce_product_name" type="text"
                    ...
                    <div class="col-span-12">
                        <label for="modal-produce-qty" class="form-label">Quantity</label>
                        <input id="modal-produce-qty" name="produce_quantity" type="number" class="form-control"
                            placeholder="Enter quantity of product to produce" required>
                    </div>
                </div>
                <!-- END: Modal Body -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (productSearchInput) {
                initProductSearch(productSearchInput);
            }

            // START:Raw product quantity modal
            const submitBtn = document.getElementById('submit-btn');
            const modal = document.getElementById('produce-qty-modal');
            const cancelBtn = document.getElementById('cancel-modal');
            const saveBtn = document.getElementById('save-modal');
            const mainForm = document.querySelector('form[action="{{ route('many-to-one.store') }}"]');
            const productSearch = document.getElementById('product_search');
            const modalProductName = document.getElementById('modal-produce-product-name');
            const modalProduceQty = document.getElementById('modal-produce-qty');
            ...
            submitBtn.addEventListener('click', function(e) {
                ...
                modalProductName.value = productSearch.value;

                showModal();

                setTimeout(() => {
                    modalProduceQty.focus();
                }, 100);
            });
            ...
            saveBtn.addEventListener('click', function() {
                const quantity = parseFloat(modalProduceQty.value);
        ...
        function initProductSearch(productSearchInput) {
            ...
            function selectProduct(product) {
                ...
                if (productSearchInput.id === 'product_search') {
                    document.getElementById('product_quantity').value = product.available_qty || 0;
                    document.getElementById('product_unit').value = product.unit_types || 'PCS';
                }
        ...
        function setupEnterNavigation() {
            ...
            const productFields = [
                'input[name="raw_product_search[]"]',
                'input[name="raw_quantity[]"]',
            ];
                ...
                if (e.key === 'Enter') {
                    ...
                    if (fieldIndex === 1) {
                        ...
                        if (isLastRow) {
                            ...
                            if (nextRow) {
                                const nextProductSearch = nextRow.querySelector('input[name="raw_product_search[]"]');
            ...
            function addProductRow() {
                ...
                const productSearch = newRow.querySelector('input[name="raw_product_search[]"]');
                const productId = newRow.querySelector('input[name="raw_product_id[]"]');
                const quantityInput = newRow.querySelector('input[name="raw_quantity[]"]');
                const dropdown = newRow.querySelector('.product-dropdown');

                if (productSearch) {
                    productSearch.id = `raw_product_search_${rowCount}`;
                    productSearch.dataset.initialized = 'false';
                }
                if (productId) {
                    productId.id = `raw_product_id_${rowCount}`;
                }
                if (quantityInput) {
                    quantityInput.id = `raw_quantity_${rowCount}`;
                }
                if (dropdown) {
                    dropdown.id = `raw_product_dropdown_${rowCount}`;
                }
            ...
            document.addEventListener('keydown', function(e) {
                ...
                if (modal && modal.classList.contains('show')) {
                    ...
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        if (document.activeElement === modalProduceQty) {
