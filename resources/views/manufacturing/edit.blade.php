...
<div class="content">
    ...
    <form action="{{ route('manufacturing.update', $manufacturing->id) }}" method="POST">
        @csrf
        @method('PUT')
                ...
                <div class="input-form col-span-3 ">
                    ...
                    <div class="product-search-container" style="position: relative;">
                        <input id="product_search" type="text" name="product_search" class="form-control field-new product_search"
                            ...
                            value="{{ $manufacturing->product->product_name ?? '' }}">
        ...
        <div class="block w-full">
            <table class="w-full mb-4 bg-transparent table-striped mt-4" id="billTbl">
                ...
                <tbody class="quote-item-container table-body">
                    @php
                        $ingredients = json_decode($manufacturing->ingredients, true) ?? [];
                    @endphp
                    @foreach ($ingredients as $index => $ingredient)
                        <tr class="border-b text-gray-700 uppercase text-sm quote-item-row" data-index="{{ $index }}">
                            <th scope="col" class="required">
                                <div class="product-search-container" style="position: relative;">
                                    <input id="ingredient_product_search_{{ $index }}" type="text" name="product_search[]" class="form-control field-new product_search"
                                        placeholder="Type to search products..." autocomplete="off" required
                                        value="{{ $ingredient['product_id'] }}">
                                    <input type="hidden" id="ingredient_product_id_{{ $index }}" name="ingredient_product_id[]" class="product_id" required
                                        value="{{ $ingredient['product_id'] }}">
                            ...
                            <th scope="col" class="required">
                                <input type="number" name="ingredient_qty[]" id="ingredient_qty_{{ $index }}" class="form-control qty" required min="0"
...
document.addEventListener('DOMContentLoaded', function() {
    var rowIndex = {{ count($ingredients) - 1 }};
