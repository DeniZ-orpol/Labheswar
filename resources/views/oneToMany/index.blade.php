...
@section('content')
    @php
        $isPaginated = method_exists($oneToMany, 'links');
    @endphp
    <div class="content">
        ...
        <div class="grid grid-cols-12 gap-6 mt-5 grid-updated">
            <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                <a href="{{ route('one-to-many.create') }}" class="btn btn-primary shadow-md mr-2 btn-hover">New Conversion</a>
            </div>
              ...
              <div id="scrollable-table" style="max-height: calc(100vh - 200px); overflow-y: auto; border: 1px solid #ddd;">
                    <table id="DataTable" class="display table table-bordered intro-y col-span-12">
                        ...
                        <tr class="bg-primary text-white">
                            ...
                            <th class="px-4 py-2">Raw Item</th>
                            <th class="px-4 py-2">Action</th>
                        </tr>
                    </thead>
                    <tbody id="data">
                        @include('oneToMany.rows', ['page' => 1])
                    </tbody>
                </table>
              </div>
                <!-- Show pagination when data is paginated -->
                {{-- @if ($isPaginated && $oneToMany->count() > 0)
                    <div class="pagination-wrapper">
                        <div class="pagination-info">
                            Showing {{ $oneToMany->firstItem() }} to {{ $oneToMany->lastItem() }} of
                            {{ $oneToMany->total() }} entries
                        </div>
                        <div class="pagination-nav">
                            <nav role="navigation" aria-label="Pagination Navigation">
                                <ul class="pagination">
                                    <!-- Previous Page Link -->
                                    @if ($oneToMany->onFirstPage())
                                        <li class="page-item disabled" aria-disabled="true">
                                            <span class="page-link">‹</span>
                                        </li>
                                    @else
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $oneToMany->previousPageUrl() }}"
                                                rel="prev">‹</a>
                                        </li>
                                    @endif

                                    <!-- Page Numbers -->
                                    @for ($i = 1; $i <= $oneToMany->lastPage(); $i++)
                                        @if ($i == $oneToMany->currentPage())
                                        ...
                                        @else
                                            <li class="page-item">
                                                <a class="page-link"
                                                    href="{{ $oneToMany->url($i) }}">{{ $i }}</a>
                                            </li>
                                        @endif
                                    @endfor

                                    <!-- Next Page Link -->
                                    @if ($oneToMany->hasMorePages())
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $oneToMany->nextPageUrl() }}"
                                                rel="next">›</a>
                                        </li>
                                    @else
                                        <li class="page-item disabled" aria-disabled="true">
...
<script>
    ...
    function fetchMoreData() {
        page++;
        fetch(`{{ route('one-to-many.index') }}?page=${page}`)
