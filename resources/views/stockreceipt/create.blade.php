...
<div class="content">
    ...
    <form action="{{ route('stockreceipt.store') }}" method="POST">
        @csrf
        ...
        <div class="row">
            <div class="column">
                ...
                <div class="mb-3">
                    ...
                    <select id="from_branch" name="from_branch" class="form-control field-new" >
            ...
            <div class="column">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control field-new" name="to_branch" id="to_branch" value="{{$branch->name}}" readonly>
                    {{-- <select id="to_branch" name="to_branch" class="form-control field-new" >
                        <option value="">Select Branch</option>
        ...
        <!-- Stock Issue Search Box -->
        <div class="row mt-4">
            <div class="column">
                <label for="stock_issue_search" class="form-label">Search Stock Issue by Issue No</label>
                <input type="text" id="stock_issue_search" class="form-control" placeholder="Enter Issue No">
            </div>
            <div class="column flex items-end">
                <button type="button" id="search_stock_issue_btn" class="btn btn-primary">Search</button>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const searchBtn = document.getElementById('search_stock_issue_btn');
        const searchInput = document.getElementById('stock_issue_search');
        const fromBranchSelect = document.getElementById('from_branch');

        searchBtn.addEventListener('click', function() {
            const issueNo = searchInput.value.trim();
            const branchId = fromBranchSelect.value;

            if (!branchId) {
                alert('Please select a From Branch before searching.');
                return;
            }

            if (!issueNo) {
                alert('Please enter an Issue No to search.');
                return;
            }

            // Call the stock issue search API
            fetch(`/stockissue/search/${branchId}?issue_no=${encodeURIComponent(issueNo)}`, {
                headers: {
            ...
            .then(data => {
                ...
                updateStockIssueTable(data.data || data);
            })
            .catch(error => {
                console.error('Error fetching stock issues:', error);
                alert('Error fetching stock issues. Please try again.');
            });
        });

        function updateStockIssueTable(stockIssue) {
            if (!stockIssue || stockIssue.length === 0) {
                alert('No Stock Issue found.');
                return;
            }

            console.log('stockIssues',stockIssue.products);
            
            // Use the first matched stock issue to fill the form
            // const stockIssue = stockIssues[0];

            // Clear existing product rows except the first
            const tableBody = document.querySelector('#billTbl tbody');
            while (tableBody.rows.length > 1) {
                tableBody.deleteRow(1);
            }

            // Fill the first row with the first product data
            const firstRow = tableBody.rows[0];
            const products = stockIssue.products || [];

            if (products.length > 0) {
                fillProductRow(firstRow, products[0]);
            }

            // Add additional rows for remaining products
            for (let i = 1; i < products.length; i++) {
                const newRow = firstRow.cloneNode(true);
                fillProductRow(newRow, products[i]);
                tableBody.appendChild(newRow);
            }

            // Fill other inputs
            document.getElementById('total-amount').value = stockIssue.total_amount || '';

            // You can fill other inputs like ledger, date, etc. if needed
        }

        function fillProductRow(row, product) {
            row.querySelector('input[name="product_search[]"]').value = product.product_search || '';
            row.querySelector('input.product_id').value = product.product_id || '';
            row.querySelector('input[name="mrp[]"]').value = product.mrp || '';
            row.querySelector('input[name="sale_rate[]"]').value = product.sale_rate || '';
            row.querySelector('input[name="qty[]"]').value = product.qty || '';
            row.querySelector('input[name="amount[]"]').value = product.amount || '';
        }
    });
</script>
<script>
